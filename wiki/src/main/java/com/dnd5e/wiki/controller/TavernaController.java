...
public class TavernaController {
	...
	@GetMapping
	public String getForm(Model model, String tavernaType, String tavernaCategory) {
		...
		do {
			...
			if (nameType > 70) {
				...
				index = rnd.nextInt(tavernaNames.size());
				TavernaName tavernaName2 = tavernaNames.get(index);
				tavernName = type.getName() + " \"" + prefix.getName()
		...
		for (int i = 0; i < visitorCount; i++) {
			List<Visitor> visit = visitors.stream()
				.flatMap(v->v.getChance().stream())
				.filter(c -> c.getChance() > rnd.nextInt(100))
				.map(c -> c.getVisitor())
				.distinct()
